...
def api_posts_get(request):
    ...
    if keyword:
        post_list = [{'name': str(post[0])} for post in list(
            RecruitPost.objects.distinct().values_list('title').filter(title__icontains=keyword))]
    else:
        post_list = []
...
def api_tags_get(request):
    ...
    if keyword:
        tag_list = [{'name': str(tags[0])} for tags in list(
            RecruitTag.objects.distinct().values_list('name').filter(name__icontains=keyword))]
    else:
        tag_list = []
...
def api_cou_get(request):
    post_list = [{'name': str(post[0])} for post in list(
        RecruitPost.objects.distinct().values_list('country').order_by('country'))]
    return JsonResponse({'object_list': post_list})
...
def api_pre_get(request):
    keyword = request.GET.get('keyword')
    if keyword:
        post_list = [{'name': str(post[0])} for post in list(RecruitPost.objects.distinct(
...
def api_cit_get(request):
    ...
    if keyword:
        post_list = [{'name': str(post[0])} for post in list(
            RecruitPost.objects.distinct().values_list('city').filter(city__icontains=keyword))]
    else:
        post_list = []
    return JsonResponse({'object_list': post_list})
    ...
    if request.method == "GET":
        ...
        if keyword_form.is_valid() and option_form.is_valid():
            ...
            if len(search_results) > 0:
                ...
                return render(request, 'search/search_recruit_by_title.html', {'latest_post_list': latest_posts, 'post_list': search_results, 'keyword_form': keyword_form, 'option_form': option_form, 'is_searched': True})
            else:
                ...
                return render(request, 'search/search_recruit_by_title.html', {'latest_post_list': latest_posts, 'keyword_form': keyword_form, 'option_form': option_form, 'is_searched': True})
    keyword_form = SearchPostKeywordForm()
    option_form = SearchPostOptionForm()
    return render(request, 'search/search_recruit_by_title.html', {'latest_post_list': latest_posts, 'keyword_form': keyword_form, 'option_form': option_form, 'is_searched': False})
...
def search(request):
    ...
    if not query_keyword_string:
        ...
        for word in query_words:
            records_per_word = []
            records_per_word += RecruitPost.objects.filter(title__icontains=word)
...
def search_tag(request):
    if request.method == "GET":
        ...
        if q:
          try:
            tag = RecruitTag.objects.get(name=q)
          except:
            tag = False
            latest_posts = get_public_post_list(RecruitPost.objects.all().order_by('-created_at')[:20])
          if tag:
            latest_posts = get_public_post_list(RecruitPost.objects.all().order_by('-created_at').filter(tags=tag)[:10])
        else:
            latest_posts = get_public_post_list(RecruitPost.objects.all().order_by('-created_at')[:20])
        tags_all = RecruitTag.objects.all()
        if tag_form.is_valid():
            try:
                tag = RecruitTag.objects.get(name=q)
                post_list = get_public_post_list(RecruitPost.objects.filter(tags=tag))
                messages.add_message(request, messages.SUCCESS, _("Your post has been found!"))
                messages.add_message(request, messages.SUCCESS, _("Up to 20 found posts will be displayed on one page"))
                return render(request, 'search/search_recruit_by_tag.html', {'latest_post_list': latest_posts, 'post_list': post_list, 'tag_form': tag_form, 'tags': tags_all, 'is_searched': True})
            except:
                messages.add_message(request, messages.SUCCESS, _("No relevant posts found."))
                ...
                return render(request, 'search/search_recruit_by_tag.html', {'latest_post_list': latest_posts, 'tag_form': tag_form, 'tags': tags_all, 'is_searched': True})
    tag_form = SearchPostTagForm()
    return render(request, 'search/search_recruit_by_tag.html', {'latest_post_list': latest_posts, 'tag_form': tag_form, 'tags': tags_all, 'is_searched': False})
